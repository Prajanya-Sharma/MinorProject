...
async function PATCH(request, { params }) {
    try {
        ...
        const { data: booking, error: fetchError } = await supabase.from("bookings").select("*").eq("id", id).eq("user_id", user.id).single();
        ...
        if (action === "end") {
            ...
            const lotResponse = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/api/lots/${booking.lot_id}`);
        ...
        const { data: updatedBooking, error: updateError } = await supabase.from("bookings").update(updateData).eq("id", id).select().single();
        if (updateError) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__25105301._.js.map