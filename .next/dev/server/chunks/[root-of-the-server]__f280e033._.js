...
async function GET(request) {
    try {
        ...
        if (isBrowse) {
            ...
            const convertedLots = data.map((lot)=>({
                    id: lot.id,
                    name: lot.name,
                    address: lot.address,
            ...
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(convertedLots, {
                status: 200
            });
        } else {
            ...
            const { data, error } = await supabase.from("parking_lots").select("*").eq("user_id", user.id).order("created_at", {
                ascending: false
            });
            if (error) {
                console.error("[v0] Supabase error:", error);
...
function convertAmenities(amenities) {
    const result = [];
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f280e033._.js.map